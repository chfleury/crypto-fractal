...
int main(int argc, char *argv[])
{
    ...
    n = atoi(argv[1]);
    altura = n;
    largura = n; // Corrected: Use n as the width (pixels per row)
    ...
    for (int i = 0; i < n; i++)
    {
        for (int j = 0; j < largura; j++)
        {
            ...
            // Calculate the index for the pixel_array correctly
            int pixel_index = 3 * (i * largura + j);

            pixel_array[pixel_index] = rgb[0];
            pixel_array[pixel_index + 1] = rgb[1];
            pixel_array[pixel_index + 2] = rgb[2];
        }
    }
